...
\begin{Description}[xxxxxxxxx]%\setlength{\itemsep}{0cm}
    ...
    \item[\OptArg{--output }{m3u\Bar xspf\Bar pls} \Arg{FILENAME}] Sets
    the output format, one of {m3u,xspf,pls}, and defines the
    \Arg{FILENAME} to save playlist to

\end{Description}

\subsection{Non-RegExp filtering options}
\begin{Description}
...
\subsection{RegExp filtering options}
\begin{Description}
    ...
    included to playlist, followed by the \% quantity (see RULES,
    REGEXP)
...
The standard PostgreSQL database structure is as following:
\begin{verbatim}
...
ALTER TABLE ONLY morceaux
    ADD CONSTRAINT pk_morceaux PRIMARY KEY (chemin);
\end{verbatim}

You can eventually change the structure and use your own, but you have
to redefine the corresponding use inside the program. Actually, there
is no simple way to do that, but alter the original source code.
